...
class Jazzifier:
    def __init__(self, piece, style = "swing", bass_max = 54, bass_min = 24, voicing_min = 38, voicing_max = 64, p_remove_note = 0.1, max_voicing_jump = 6, verbose = False):
        ...
        self.swingified_note_track = self.get_swingified_melody()
    ...
    def get_swingified_melody(self):
        ...
        self.swingified_note_track = deepcopy(self.piece.notes[:])
        
        prolong_dur_flag = False
        for i,note in enumerate(self.swingified_note_track):
            beat_time = note.time - first_downbeat_time
            if i < len(self.swingified_note_track) - 1:
                next_note_duration = self.swingified_note_track[i+1].duration
            else:
            ...
            self.swingified_note_track[i] = note
        
        return self.swingified_note_track
...
#                         rhythm_template = randomly_acticipatify_rhythm(rhythm_template, p = 0.4)
    ...
    def get_5th_root_note(self, chord_root, chord_quality):
    ...
    def get_leading_bass_note(self, next_chord_bass, p_above=0.6):
        p = np.random.random()
        if p < p_above:
    ...
    def get_root_or_5th(self, chord_root, chord_quality, p_root = 0.3):
        p = np.random.random()
        if p < p_root:
            bass_note = chord_root
        else:
            bass_note = self.get_5th_root_note(chord_root, chord_quality)
        return bass_note
    
    def get_leading_or_5th(self, next_chord_bass, chord_root, chord_quality, p_above=0.6, p_leading = 0.8):
        p = np.random.random()
        if p < p_leading:
            bass_note = self.get_leading_bass_note(next_chord_bass, p_above=p_above)
        else:
            bass_note = self.get_5th_root_note(chord_root, chord_quality)
        return bass_note
    ...
    def get_bass_pitch(self, i_chord, bass_start, prev_bass_pitch, contour, bass_max = 54, bass_min = 24):
        ...
        if i_chord == self.N_chords - 1:
            ...
            bass_pitch = self.get_root_or_5th(chord_root, chord_quality, p_root = 0.3)
                
        elif (bass_start - chord_start)%72 == 24 and chord_duration >= 24*3:
            bass_pitch = self.get_root_or_5th(chord_root, chord_quality, p_root = 0.6)
                
        elif (bass_start - chord_start)%96 == 24 and chord_duration == 24*2:
            bass_pitch = self.get_leading_or_5th(next_chord_bass, chord_root, chord_quality, p_above=0.6, p_leading = 0.8)
            ...
            if (prev_bass_pitch - chord_root)%12 == INT_5:
                ...
                bass_pitch = self.get_5th_root_note(chord_root, chord_quality)
        ...
        next_contour = self.determine_note_contour(prev_bass_pitch, bass_pitch)
...
#                         rhythm_template.insert(i_note+1, (rhythm_template[i_note][0]+16, 8))
    ...
    def output_json(self, out_path="test.json"):
        ...
        # swingified notes
        note_list_new = [None]*len(self.swingified_note_track)
        for i in range(len(self.swingified_note_track)):
            note_list_new[i] = {"pitch": self.swingified_note_track[i].pitch,
                                "duration": self.swingified_note_track[i].duration}
    ...
    def output_swingified_result(self, out_dir=""):
        ...
        notes_chord_to_midi(
            [self.swingified_note_track, self.bass_track], self.voicing_track, self.piece.time_signatures, midi_path, resolution=24
        )
